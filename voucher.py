...
def toggle_cheque_fields(index, cheque_frame, online_frame, voucher_data):
...
def get_selected_option(count, selected_option, group_frames, frame_name_options, cheque_frame, online_frame, voucher_data):
    ...
    cash = tk.Radiobutton(group_frames[frame_name_options], text="Cash", bg="#99ccff", variable=selected_option[count], value="Cash", font=font_settings, command=lambda c=count: toggle_cheque_fields(c, cheque_frame, online_frame, voucher_data))
    cheque = tk.Radiobutton(group_frames[frame_name_options], text="Cheque", bg="#99ccff", variable=selected_option[count], value="Cheque", font=font_settings, command=lambda c=count: toggle_cheque_fields(c, cheque_frame, online_frame, voucher_data))
    online = tk.Radiobutton(group_frames[frame_name_options],  text="EFT", bg="#99ccff", variable=selected_option[count], value="EFT", font=font_settings, command=lambda c=count: toggle_cheque_fields(c, cheque_frame, online_frame, voucher_data))
...
def draw_voucher(frame, selected_indx, root, checkbox_var):
    ...
    dest_excel_path = UI_support.generate_excel_file(dv_entity_xcls[selected_indx])
    ...
    for i in range(3): 
        ...
        # UI Layout
        df, codes, code_to_data = load_excel_and_generate_codes(dest_excel_path)
        #print(f"CODES {codes}")
...
def draw_inv_voucher(frame):
    for i in range(3):
        ...
        receipt_frame = tk.LabelFrame(frame, text=f"Voucher {i + 1}", font=font_settings_bold, bg="#99ccff", bd=2, relief="groove", padx=10, pady=10)

        # Place the LabelFrame in a unique row (using i * 7 to create a gap between each receipt)
        receipt_frame.grid(row=i * 7, column=0, padx=10, pady=10, sticky="ew")
        ...
        tk.Label(receipt_frame, text="Amount:", font=font_settings, bg="white").grid(row=2, column=0, sticky="w", pady=5)
