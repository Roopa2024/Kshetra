...
import canvas_update, Toggle_button_UI
...
# This is for Receipt : Creating Frames to group fields with borders
...
def button_handling(index):
    ...
    if button_names[index] != "Receipt":
        ...
        match button_names[index]:
            case 'Debit Voucher':
                dv_frame = tk.Frame(root, borderwidth=2, relief="groove")
                dv_frame.grid(row=1, column=0, padx=50, pady=20, sticky="n")
                Toggle_button_UI.debit_voucher(dv_frame, font_settings_bold, font_settings)
            case 'Invoice Voucher':
                Toggle_button_UI.invoice_voucher(root)
            case 'Purchase Voucher':
                Toggle_button_UI.purchase_voucher(root)
            case 'Service Voucher':
                Toggle_button_UI.service_voucher(root)
