...
# Create the dictionary in a single line
headings = {key: config.get('Heading', key) for key in ['heading', 'sub_heading', 'pdf_heading']}
columns = {key: config.get('Columns', key) for key in ['columns', 'column_names', 'column_names_kartha']}
...
def underline_text(canvas, x_pos, y_pos, text):
    ...
    if text in headings or text in headings['pdf_heading']:
...
def wrap_text(c, text, x, y, width, font_name, font_size):
    ...
    paragraph.drawOn(c, x, y)           # Draw the wrapped text on the canvas at (x, y)
...
for index, row in df.iterrows():
    ...
    create_pdf_for_row(row, pdf_file_name, int(pdf_type)) # pdf_type = 0(SC)/1(PC)/2(IC) 
